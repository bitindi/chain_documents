...
Note that the value of the `ethereum` field in `docker-compose.yml` is replaced with `<<XYX>>`'s node connection information.
...
graph-node:
    ...
    environment:
      ...
      ipfs: 'ipfs:5001'
      ethereum: 'mainnet:http://127.0.0.1:8545'  #replaced with <<XYX>> rpc info
      RUST_LOG: info
 ```
