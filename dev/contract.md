# Contract
`<<XYX>>` use `EVM` for contract execution.See [Solidity](https://docs.soliditylang.org/en/v0.7.1/#) for detail.
...
And then by `MetaMask` the contract deployemnt transaction can be broadcast to the BITINDI Chain.
...
module.exports = {
  networks: {
    testnet: {
      provider: () => new HDWalletProvider(mnemonic, 'https://http-testnet.bitindi.org'),
      network_id: 256
    },
    mainnet: {
      provider: () => new HDWalletProvider(mnemonic, 'https://http-mainnet.bitindi.org'),
      network_id: 128
    }
  },
